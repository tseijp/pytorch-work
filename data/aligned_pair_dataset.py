...
class AlignedPairDataset(BaseDataset):
    def initialize(self, opt):
        ...
        if not opt.no_instance:
            ...
            self.inst_paths = sorted(image_folder.make_dataset(self.dir_inst))

        ### load precomputed instance-wise encoded features
        if opt.load_features:
            self.dir_feat = os.path.join(opt.dataroot, opt.phase + '_feat')
            print('----------- loading features from %s ----------' % self.dir_feat)
            self.feat_paths = sorted(image_folder.make_dataset(self.dir_feat))
    ...
    def __getitem__(self, index):
        ...
        params = image_transform.get_params(self.opt, A[0].size)
        if self.opt.label_nc == 0:
            transform_A = image_transform.get_transform(self.opt, params)
            A_tensor = [transform_A(item.convert('RGB')) for item in A]
            A_tensor = torch.stack(A_tensor, dim=0)
        else:
            transform_A = image_transform.get_transform(self.opt, params, method=Image.NEAREST, normalize=False)
        ...
        if self.opt.isTrain:
            ...
            transform_B = image_transform.get_transform(self.opt, params)
        ...
        if not self.opt.no_instance:
            ...
            if self.opt.load_features:
                ...
                norm = image_transform.normalize()
