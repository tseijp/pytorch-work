    ...
    def __call__(self, input, target_is_real):
        if isinstance(input[0], list):
            ...
            for input_i in input:
                pred = input_i[-1]#.to('cuda:0')
                target_tensor = self.get_target_tensor(pred, target_is_real)#.to('cuda:0')# my changed 
                loss += self.loss(pred, target_tensor) # error position
            return loss
        else:            
...
class LocalEnhancer(nn.Module):
    ...
    def forward(self, input):
        ...
        output_prev = self.model(input_downsampled[-1])#.to('cuda:0'))    # error position my changed
        ### build up one layer at a time
        for n_local_enhancers in range(1, self.n_local_enhancers+1):
            model_downsample = getattr(self, 'model'+str(n_local_enhancers)+'_1')
            model_upsample = getattr(self, 'model'+str(n_local_enhancers)+'_2')            
            input_i = input_downsampled[self.n_local_enhancers-n_local_enhancers]           
            output_prev = model_upsample(model_downsample(input_i) + output_prev) # my changed
        return output_prev
...
class MultiscaleDiscriminator(nn.Module):
    ...
    def forward(self, input):        
        ...
        for i in range(num_D):
            ...
            result.append(self.singleD_forward(model, input_downsampled))#.to('cuda:0'))) #error position
            if i != (num_D-1):
...
class Vgg19(torch.nn.Module):
    ...
    def forward(self, X):
        h_relu1 = self.slice1(X)#.to('cuda:0'))
